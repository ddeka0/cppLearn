...
struct PrintItem<A,MT,MPTR,1> {
	static void Print(const A& v,const std::string& key,int offset) {
...
void PrintPack(const A &v,Pack<MemberBinding<MT,MPTR,Ndx>, Rest...> * metadata,int offset = 0) {
	using M = Members<A>;
...
template<typename A>
void PrintPack(const A &v, Pack<> * metadata,int offset = 0) {}
